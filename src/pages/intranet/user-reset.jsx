...
export async function getServerSideProps(context) {
    ...
    try {
      const req = await apiAxios.get('/user/modules-roles',{headers,params:{url:'/user-reset'}});
    ...
    <div className='bg-layout p-5 flex justify-center items-center min-h-dvh'>
        <div className='bg-white px-4 py-7 max-w-md rounded-lg shadow-md'>
            <form onSubmit={handleSubmit}>
                ...
                <InputLogin label="Contraseña" type="password" name="password_1" value={form.password_1||''} onChange={handleChangeForm}/>
                <InputLogin label="Repetir contraseña" type="password" name="password_2" value={form.password_2||''} onChange={handleChangeForm}/>
                <div className="mb-1 text-center">
                    <ButtonPrimary text="Ingresar" type='submit' icon={<ArrowRightEndOnRectangleIcon className='w-6 h-6'/>}/>
                    <ButtonDanger text="Salir" icon={<ArrowLeftEndOnRectangleIcon className='w-6 h-6'/>} onClick={handleCloseSession}/>
                </div>
            </form>
        </div>
    </div>
