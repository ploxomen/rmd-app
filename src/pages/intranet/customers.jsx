...
import {
  TYPES_CUSTOMERS,
  customersIntialState,
  reducerCustomers,
} from "@/reducers/crudCustomers";
...
function customers({ dataModules, dataUser, dataRoles }) {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        sweetAlert({
          title: "Error",
          text: "Error al obtener los tipos de documentos o departamentos",
          icon: "error",
        });
      }
    };
  ...
  const handleSaveCustomer = async (form, contacts) => {
    try {
      const resp = !form.id
        ? await apiAxios.post("/customer", { ...form, contacts }, { headers })
        : await apiAxios.put(
            "/customer/" + form.id,
            { ...form, contacts },
            { headers },
      ...
      sweetAlert({
        title: "Error",
        text: "Error alguardar los datos del cliente",
        icon: "error",
      });
    }
  };
  const getCustomer = async (idCustomer) => {
    try {
      ...
      sweetAlert({
        title: "Error",
        text: "Error al obtener al cliente",
        icon: "error",
      });
    }
  };
  ...
  const handleDeleteCustomer = async (idCustomer) => {
    ...
    try {
      const resp = await apiAxios.delete(`/customer/${idCustomer}`, {
        headers,
      });
      if (resp.data.redirect !== null) {
        return route.replace(resp.data.redirect);
  ...
  const exportProducts = async () => {
    downloadFiles("/customer/export/excel", "clientes", {
      search: dataChange.search,
    });
  };
  return (
    <>
      ...
      >
        <BanerModule
          imageBanner="/baners/Group 12.jpg"
      ...
      <FormCustomer
        ...
        contries={state.contries}
        typeDocumentsData={state.typeDocuments}
        statusModal={modal}
        customerEdit={state.customerEdit}
        contactsEdit={state.customerContactsEdit}
        pronvincesData={state.provinces}
        districtsData={state.districts}
        departamentsData={state.departaments}
