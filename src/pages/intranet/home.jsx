...
export async function getServerSideProps(context) {
  ...
  try {
    // Hacer la solicitud lo más rápido posible para verificar el estado de redirección
    const req = await apiAxios.get("/user/modules-roles", {
      headers,
      params: { url: "/home" },
    });
    // Si hay una redirección, hacerla inmediatamente
    ...
    // Si no hay redirección, continuar con los datos
    return {
      props: {
...
export default function Home({ dataModules, dataRoles, dataUser }) {
  ...
  return (
    <LoyoutIntranet
      title="Inicio"
      description="Inicio de la intranet"
      user={dataUser}
      modules={dataModules}
      roles={dataRoles}
    >
      <div className="grid grid-cols-12 gap-4 mb-6">
        <CartHome
          title="Clientes"
          quantity={limit.customersCount}
          backgroundIcon="bg-violet-200"
          Icon={UserGroupIcon}
          colorIcon="text-violet-500"
        />
        <CartHome
          title="Cotizaciones"
          quantity={limit.quotationsCount}
          backgroundIcon="bg-yellow-200"
          Icon={DocumentChartBarIcon}
          colorIcon="text-yellow-500"
        />
        <CartHome
          title="Cotizaciones aprobadas"
          quantity={limit.quotationsCheckCount}
          backgroundIcon="bg-green-200"
          Icon={DocumentCheckIcon}
          colorIcon="text-green-500"
        />
        <CartHome
          title="Usuarios"
          quantity={limit.usersCount}
          backgroundIcon="bg-orange-200"
          Icon={UserIcon}
      ...
      <div className="p-6 bg-white rounded-md shadow">
        <div className="grid grid-cols-12 gap-10 mb-6 overflow-x-auto">
          ...
          <div className="col-span-full lg:col-span-6">
            <BarLine
              data={charts.quotationsCheckBar}
              titleLabel="Cotizaciones aprobadas"
            />
          </div>
          <div className="col-span-full lg:col-span-6">
            <BarDought
              data={charts.productsSale}
              titleLabel="Productos más vendidos"
            />
          </div>
        </div>
      </div>
