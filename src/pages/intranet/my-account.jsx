...
function MyAccount({ dataUser, dataModules, dataRoles }) {
  ...
  const filterLengthDocuments = typeDocumentsData.find(
    (typeDocument) => typeDocument.id == form.user_type_document
  );
  ...
  useEffect(() => {
    const getRoles = async () => {
      try {
        ...
        const img = resp.data.data.user_avatar
          ? process.env.NEXT_PUBLIC_API_URL + "/" + resp.data.data.user_avatar
          : null;
        ...
        sweetAlert({
          title: "Error",
          text: "Error al obtener los tipos del usuario",
          icon: "error",
        });
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      sweetAlert({
        title: "Error",
        text: "Error al actualizar los datos de mi perfil",
        icon: "error",
      });
    }
  };
  return (
    <LoyoutIntranet
      title="Mi perfil"
      description="Configuración de datos de mi perfil"
      user={dataUser}
      modules={dataModules}
      roles={dataRoles}
    >
      <form
        className="w-full p-6 mb-4 bg-white rounded-md shadow grid grid-cols-12 gap-x-3 gap-y-0"
      ...
      >
        ...
        <div className="col-span-full flex gap-2 items-center mb-3">
          ...
          />
          <img
            src={form.user_avatar ? form.user_avatar : "/img/user.png"}
            alt="Imagen avatar"
            width={100}
            height={100}
          />
        ...
        <div className="col-span-full md:col-span-6 lg:col-span-3">
          <SelectPrimary
            ...
            name="user_type_document"
            value={form.user_type_document || ""}
            onChange={handleChangeForm}
          >
        ...
        <div className="col-span-full md:col-span-6 lg:col-span-3">
          <InputPrimary
            label="N° Documento"
            name="user_number_document"
            minLength={digitDocuments.min}
            maxLength={digitDocuments.max}
            disabled={
              digitDocuments.min && digitDocuments.max ? "" : "disabled"
            }
            inputRequired={digitDocuments.min && "required"}
            value={form.user_number_document || ""}
            onChange={handleChangeForm}
          />
        </div>
        <div className="col-span-full md:col-span-6">
        ...
        <div className="col-span-full md:col-span-6 lg:col-span-4">
          <SelectPrimary
            label="Género"
            name="user_gender"
            value={!form.user_gender ? "N" : form.user_gender}
            onChange={handleChangeForm}
          >
