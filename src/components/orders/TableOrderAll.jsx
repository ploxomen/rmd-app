...
function TableOrderAll({ orders, deleteOrder, getOrder, status }) {
  ...
  const downloadFileOs = async (idOrder, fileName) => {
    ...
    try {
      const response = await apiAxios.get(
        `order-extra/download-os/${idOrder}`,
        {
          responseType: "blob",
          headers,
        },
      ...
      if (error.response.status === 404) {
        sweetAlert({
          title: "Error",
          text: "El archivo de la OS no fue encontrado",
          icon: "error",
        });
      }
  ...
  const downloadPdf = async (id, fileName) => {
    ...
    try {
      ...
      sweetAlert({
        title: "Error",
        text: "Error al descargar el pedido",
        icon: "error",
      });
    }
  };
      ...
      {!orders.length ? (
        ...
        orders.map((order) => (
          <tr className="bg-white dark:bg-gray-800" key={order.id}>
            ...
            <td className="p-1">
              {parseMoney(order.order_mount, order.order_money)}
            </td>
            <td className="p-1">
              {parseMoney(order.order_mount_igv, order.order_money)}
            </td>
            <td className="p-1">
              {parseMoney(order.order_total, order.order_money)}
            </td>
            ...
            <td className="p-1 text-center">
              {order.order_status > 0 ? (
                <div className="flex gap-1 flex-wrap justify-center">
                  ...
                  <ButtonDangerSm
                    ...
                    onClick={(e) =>
                      downloadPdf(
                        order.id,
                        `${order.order_code}_${order.customer_name.replace(/ /g, "_").toUpperCase()}.pdf`,
                      )
                    }
                  />
                  <Dropdown
                    ...
                    options={[
                      ...
                      <button
                        ...
                        onClick={(e) =>
                          downloadFileOs(order.id, order.order_file_name)
                        }
                      >
                        Descargar OS
                      </button>,
                ...
                <strong className="text-red-500">Sin acciones</strong>
              )}
            </td>
          </tr>
        ))
